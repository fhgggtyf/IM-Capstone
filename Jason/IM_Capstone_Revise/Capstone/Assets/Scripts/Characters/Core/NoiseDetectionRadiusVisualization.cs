...
public class NoiseDetectionRadiusVisualization : CoreComponent
{
    ...
    private NonPlayerStatsManager _stats;   // <-- add this
    ...
    protected override void Awake()
    {
        ...
        if (material != null)
            ...
            Debug.LogWarning($"{nameof(NoiseDetectionRadiusVisualization)}: No material assigned. " +
        ...
        // Find the stats manager on THIS enemy instance.
        // If StatsManager is on the root (common), this works:
        _stats = GetComponentInParent<NonPlayerStatsManager>();
        if (_stats == null)
            Debug.LogError($"{nameof(NoiseDetectionRadiusVisualization)}: Could not find NonPlayerStatsManager in parents.");
    }
    ...
    private void OnEnable()
    {
        if (_stats != null)
        {
            _stats.SoundDetectionLevelChanged += OnNoiseRadiusChanged;

            // Optional: sync current value immediately on enable
            OnNoiseRadiusChanged(_stats.SoundDetectionLevel);
        }
    }
    ...
    {
        Debug.Log($"NoiseDetectionRadiusVisualization: Noise radius changed to {radius}");
