...
public class InteractableDetector : CoreComponent
{
    ...
    private IInteractable lockedInteractable;

    public void SetLockedInteractable(IInteractable interactable)
    {
        // clear any highlighting on the old lock when unlocking
        if (interactable == null && lockedInteractable != null)
            lockedInteractable.DisableInteraction();
    ...
    public void TryInteract()
    {
        ...
        {
            OnTryInteract?.Invoke(lockedInteractable);
            return;
        }
    ...
    private void Update()
    {
        // if locked, keep it selected and skip scanning
        if (lockedInteractable != null)
        {
            if (closestInteractable != lockedInteractable)
            {
                closestInteractable?.DisableInteraction();
                closestInteractable = lockedInteractable;
